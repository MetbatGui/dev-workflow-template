...
class FakeHttpClient:
    """외부 HTTP 클라이언트 자리에 넣는 가벼운 진짜 구현. 큐에 넣은 응답을 순서대로 돌려주고,
...
# 테스트에서
fake = FakeHttpClient([ConnectionError(), ConnectionError(), data_ok])
client = SomeExternalApiClient(http=fake, sleep=lambda s: None)

result = client.fetch(some_id)

assert result == expected_value          # State Verification: 결과값
assert len(fake.calls) == 3              # Fake 자신의 상태를 확인 (Mock.call_count 아님)
```
...
마커는 `pyproject.toml`의 `[tool.pytest.ini_options] markers`에 등록한다. `pytest -m unit`처럼 타입별 실행 가능.
...
tests/
  news/
    test_get_news_acceptance.py   # @pytest.mark.acceptance
    test_news_repository.py       # @pytest.mark.unit
```
...
**주의**: 테스트 디렉토리에 `__init__.py`를 만들지 않는다. 소스 패키지명과 테스트 하위 디렉토리명이 우연히 겹치면(예: 소스 패키지 `future` + `tests/future/__init__.py`), pytest의 "prepend" import mode가 테스트 디렉토리를 진짜 패키지보다 먼저 `sys.path`에 등록해 `ModuleNotFoundError`가 난다. `--import-mode=importlib`로도 해결 안 될 수 있으니, 애초에 테스트 하위 디렉토리에 `__init__.py`를 두지 않는 게 가장 확실하다.
